...
import {
  ...
  defaultScalarTransformParams,
  scalarTransformOptions,
} from "./transformDefaults";
import { scalarTransformHelp } from "./transformHelp";
import type { ScalarSheet, ScalarTransformDraft, ScalarTransformType } from "./types";
...
type ParamsEditorProps = {
  draft: ScalarTransformDraft;
  sheet: ScalarSheet;
  onChange: (params: Record<string, any>) => void;
};

function ScalarParamsEditor({ draft, sheet, onChange }: ParamsEditorProps) {
  ...
  return (
    <div className="space-y-2">
      <label className="block">
        <span className="mb-1 block text-xs font-medium text-slate-600">referenceMetricId</span>
        <select
          value={String(params.referenceMetricId ?? "")}
          onChange={(event) => onChange({ ...params, referenceMetricId: event.target.value })}
          className="w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none transition focus:border-sky-500 focus:ring-2 focus:ring-sky-100"
        >
...
type ScalarTransformPanelProps = {
  sheet: ScalarSheet;
};

export function ScalarTransformPanel({ sheet }: ScalarTransformPanelProps) {
  ...
  const [type, setType] = useState<ScalarTransformType>("scale");
  const activeMetric = useMemo(
    () => sheet.metrics.find((metric) => metric.id === sheet.activeMetricId),
    [sheet.activeMetricId, sheet.metrics],
  );
  ...
  return (
    <aside className="flex h-full min-h-0 flex-col border-l border-slate-200 bg-slate-50">
      <div className="border-b border-slate-200 px-4 py-3">
        <h2 className="text-sm font-semibold text-slate-900">Scalar Pipeline</h2>
        <p className="mt-1 truncate text-xs text-slate-500">
          Active: {activeMetric ? activeMetric.name : "未选择"}
        </p>
        {canApply ? <p className="mt-1 text-xs text-sky-700">指标卡片正在实时预览当前 Pipeline。</p> : null}
      </div>
      ...
      <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-4">
        <section className="space-y-3 rounded-md border border-slate-200 bg-white p-3">
          <div className="flex items-center justify-between">
            ...
            <HelpHint help={scalarTransformHelp[type]} />
          </div>
          <select
            value={type}
            onChange={(event) => setType(event.target.value as ScalarTransformType)}
            className="w-full rounded-md border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 outline-none transition focus:border-sky-500 focus:ring-2 focus:ring-sky-100"
          >
            {scalarTransformOptions.map((option) => (
            ...
            onClick={() =>
              updateScalarSheet(sheet.id, (current) => ({
                ...
                transformDrafts: [
                  ...
                  createScalarTransformDraft(type, current.referenceMetricId),
                ],
              }))
            }
                ...
                <div key={draft.id} className="space-y-3 rounded-md border border-slate-200 bg-white p-3">
                  <div className="flex items-center gap-2">
                    ...
                    <select
                      value={draft.type}
                      onChange={(event) => {
                        const nextType = event.target.value as ScalarTransformType;
                        updateScalarSheet(sheet.id, (current) => ({
                          ...current,
                          transformDrafts: current.transformDrafts.map((item) =>
                            item.id === draft.id
                              ? {
                                  ...item,
                                  type: nextType,
                                  params: defaultScalarTransformParams(nextType, current.referenceMetricId),
                                }
                              : item,
                          ),
                        }));
                    ...
                    <HelpHint help={scalarTransformHelp[draft.type]} />
                  </div>
                  <ScalarParamsEditor
                    draft={draft}
                    sheet={sheet}
      ...
      <div className="space-y-2 border-t border-slate-200 p-4">
        {hasInvalidReferenceStep ? (
          <div className="rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-700">
            Reference Based step 需要选择 reference metric。
          </div>
        ) : null}
        <button
          type="button"
          disabled={!canApply}
          onClick={() => updateScalarSheet(sheet.id, applyScalarPipeline)}
        ...
        <div className="grid grid-cols-2 gap-2">
          <button
            ...
            onClick={() => updateScalarSheet(sheet.id, (current) => ({ ...current, transformDrafts: [] }))}
